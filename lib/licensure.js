...
var createCSV = function (licenses) {
    var csv = [];
...
if (cli.text) {
    ...
    findLicenses(cli.unique, cli.licenseText).then(function (data) {
        fs.writeFile(path.join(process.cwd(), 'licensure.csv'), createCSV(data), function (err) {
            if (err) {
                throw err;
            }

            console.log('Finished processing ' + data.length + ' licenses.');
            console.log('License information store in licensure.csv.');
        });
    });
} else {
    findLicenses(cli.unique, cli.licenseText).then(function (data) {
